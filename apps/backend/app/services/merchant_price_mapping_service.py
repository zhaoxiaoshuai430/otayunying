    ...
    db.execute(
        text(
            ...
            CREATE TABLE IF NOT EXISTS merchant_price_mappings (
              ...
              last_seen_price DECIMAL(10, 2) NULL,
...
def _mapping_row_to_public(row: dict) -> dict:
    ...
    return {
        ...
        'last_seen_price': float(row['last_seen_price']) if row.get('last_seen_price') is not None else None,
    ...
    row = db.execute(
        text(
            ...
            SELECT id, tenant_id, shop_id, platform, room_name, rate_name, merchant_room_key,
                   merchant_rate_key, gid, hid, status, notes, last_seen_price, last_seen_at
...
def upsert_merchant_price_mapping(db: Session, *, mapping_data: dict) -> dict:
    ...
    last_seen_price = mapping_data.get('last_seen_price')
    payload = {
        'tenant_id': tenant_id,
        'shop_id': shop_id,
        'platform': platform,
        ...
        'last_seen_price': round(float(last_seen_price), 2) if last_seen_price not in (None, '') else (existing_row or {}).get('last_seen_price'),
    }
            ...
            db.execute(
                text(
                    ...
                    (tenant_id, shop_id, platform, room_name, rate_name, merchant_room_key, merchant_rate_key,
                     gid, hid, status, notes, last_seen_price, last_seen_at, created_at, updated_at)
                    VALUES
                    (:tenant_id, :shop_id, :platform, :room_name, :rate_name, :merchant_room_key, :merchant_rate_key,
                     :gid, :hid, :status, :notes, :last_seen_price, NOW(), NOW(), NOW())
                    """
                ),
                payload,
            )
