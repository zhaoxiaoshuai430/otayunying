...
def _normalize_fliggy_collect_mode(raw_mode: str | None) -> str:
    ...
    if normalized in {_FLIGGY_COLLECT_MODE_CDP_CURRENT_PAGE, _FLIGGY_COLLECT_MODE_STORAGE_STATE}:
        return normalized
...
def _resolve_fliggy_guest_state_path(*, shop_id: int, storage_state_name: str | None = None) -> tuple[str, Path]:
    state_name = _safe_storage_state_name(storage_state_name, shop_id=shop_id)
    return state_name, _fliggy_guest_state_dir() / state_name
...
def _is_windows_dpapi_unavailable(exc: Exception) -> bool:
    return "Windows DPAPI" in str(exc)


def _save_fliggy_guest_credential_if_supported(
    db: Session,
    *,
    tenant_id: int,
    shop_id: int,
    ...
    try:
        ...
        if _is_windows_dpapi_unavailable(exc):
            save_merchant_credential(
                db=db,
                credential_data={
                    "tenant_id": tenant_id,
...
def login_fliggy_guest_session(
    ...
    *,
    ...
    should_load_saved_secret = not (supplied_username and supplied_password)
    saved_credential = get_merchant_credential(
        db=db,
        shop_id=shop_id,
        platform=_FLIGGY_GUEST_PLATFORM,
        include_secret=should_load_saved_secret,
    )
    ...
    resolved_username = str(supplied_username or saved_credential.get("username") or "").strip()
    resolved_password = str(supplied_password or saved_credential.get("password") or "")
    ...
    if resolved_username:
        credential_saved, credential_save_error = _save_fliggy_guest_credential_if_supported(
            db=db,
            tenant_id=tenant_id,
            shop_id=shop_id,
    ...
    return {
        ...
        "credential_save_error": credential_save_error,
    }
...
def collect_fliggy_hotel_prices_playwright(
    *,
    ...
    """Collect Fliggy hotel price rows via a logged-in browser page or saved storage state."""
    ...
    resolved_storage_state_name = str(storage_state_name or "").strip()
    ...
    if db is not None:
        context_data = _resolve_fliggy_guest_context(db=db, shop_id=shop_id)
        should_load_saved_secret = not (resolved_username and resolved_password)
        secret_credential = get_merchant_credential(
            db=db,
            shop_id=shop_id,
            tenant_id=int(context_data["credential"].get("tenant_id") or 1),
            platform=_FLIGGY_GUEST_PLATFORM,
        ...
        resolved_storage_state_name = resolved_storage_state_name or str(context_data["storage_state_name"] or "").strip()
        resolved_username = resolved_username or str(secret_credential.get("username") or "").strip()
        resolved_password = resolved_password or str(secret_credential.get("password") or "")
        ...
        if save_credential and resolved_username:
            credential_saved, credential_save_error = _save_fliggy_guest_credential_if_supported(
                db=db,
                tenant_id=int(context_data["credential"].get("tenant_id") or 1),
                shop_id=shop_id,
    ...
    state_path = None
    if shop_id >= 1:
        _, state_path = _resolve_fliggy_guest_state_path(
            shop_id=shop_id,
            storage_state_name=resolved_storage_state_name,
        )

    def _run_collect() -> dict:
        if resolved_collect_mode == _FLIGGY_COLLECT_MODE_STORAGE_STATE:
            return _collect_fliggy_hotel_prices_playwright_once(
                shop_id=shop_id,
                start_url=resolved_start_url,
                max_pages=max_pages,
                max_hotels=max_hotels,
        ...
        return _collect_fliggy_hotel_prices_via_cdp(
            shop_id=shop_id,
            start_url=resolved_start_url,
            max_pages=max_pages,
            max_hotels=max_hotels,
    ...
    try:
        ...
        if not auto_login or db is None or message not in _GUEST_SESSION_LOGIN_REQUIRED_ERRORS:
            raise
    ...
    result["auto_login_performed"] = auto_login_performed
    result["credential_saved"] = credential_saved
    result["credential_save_error"] = credential_save_error
...
def _crawl_hotel_room_prices_once(
    *,
    hotel_name: str,
    hotel_url: str,
    headless: bool = True,
    debug_url: str | None = None,
    collect_mode: str = _FLIGGY_COLLECT_MODE_PREFER_CDP,
    storage_state_path: Path | None = None,
    login_url: str | None = None,
    selectors: dict | str | None = None,
) -> dict:
    ...
    with sync_playwright() as p:
        ...
        temp_page = None
        try:
            if resolved_collect_mode == _FLIGGY_COLLECT_MODE_CDP_CURRENT_PAGE and _looks_like_fliggy_detail_url(hotel_url):
                try:
            ...
            if page is None:
                ...
                if storage_state_path is not None and storage_state_path.exists():
                    context_kwargs["storage_state"] = str(storage_state_path)
                local_context = local_browser.new_context(**context_kwargs)
    ...
    return normalize_crawled_room_price_result(
        {
...
def crawl_hotel_room_prices(
    *,
    ...
    resolved_username = str(username or "").strip()
    resolved_password = str(password or "")
    resolved_shop_id = int(shop_id or 0)
    state_path = None

    if db is not None and resolved_shop_id >= 1:
        context_data = _resolve_fliggy_guest_context(db=db, shop_id=resolved_shop_id)
        secret_credential = get_merchant_credential(
            db=db,
        ...
        resolved_login_url = resolved_login_url or str(context_data["login_url"] or "").strip()
        resolved_selectors = resolved_selectors if resolved_selectors not in (None, "") else context_data["selectors"]
        resolved_storage_state_name = resolved_storage_state_name or str(context_data["storage_state_name"] or "").strip()
        resolved_username = resolved_username or str(secret_credential.get("username") or "").strip()
        resolved_password = resolved_password or str(secret_credential.get("password") or "")

    if resolved_shop_id >= 1:
        _, state_path = _resolve_fliggy_guest_state_path(
            shop_id=resolved_shop_id,
            storage_state_name=resolved_storage_state_name,
        )
    elif resolved_collect_mode == _FLIGGY_COLLECT_MODE_STORAGE_STATE:
        raise ValueError("shop_id is required for storage_state room crawl")

    try:
        return _crawl_hotel_room_prices_once(
            hotel_name=hotel_name,
            hotel_url=hotel_url,
            headless=headless,
            debug_url=debug_url,
        ...
        message = str(exc or "").strip().lower()
        if not auto_login or db is None or resolved_shop_id < 1 or message not in _GUEST_SESSION_LOGIN_REQUIRED_ERRORS:
            raise
        login_fliggy_guest_session(
            db=db,
            shop_id=resolved_shop_id,
...
def normalize_crawled_room_price_result(
    result: dict | None,
    *,
    ...
    resolved_hotel_name = str(hotel_name or payload.get('hotel_name') or '').strip() or 'unknown'
    resolved_hotel_url = str(hotel_url or payload.get('hotel_url') or '').strip()
    collected_at = str(payload.get('collected_at') or datetime.now().strftime("%Y-%m-%d %H:%M:%S")).strip()
    ...
    if isinstance(raw_rooms, list):
        for room in raw_rooms:
            ...
            normalized_rooms.append(
                {
                    'room_type': str(room.get('room_type', '')).strip()[:64],
                    'rate_name': str(room.get('rate_name', '')).strip()[:64],
                    'price': price,
                    'breakfast': str(room.get('breakfast', '未知')).strip()[:16] or '未知',
                    'cancelable': str(room.get('cancelable', '未知')).strip()[:16] or '未知',
                    'raw_text': str(room.get('raw_text', room.get('raw', ''))).strip()[:300],
                }
            )

    normalized = {
        'hotel_name': resolved_hotel_name,
        'hotel_url': resolved_hotel_url,
        'collected_at': collected_at,
        ...
        if not name or not url:
            results.append(
                normalize_crawled_room_price_result(
                    {"hotel_name": name or "unknown", "hotel_url": url, "error": "\u7f3a\u5c11\u540d\u79f0\u6216URL", "rooms": []}
                )
            )
            continue
        try:
            result = crawl_hotel_room_prices(
                hotel_name=name,
                hotel_url=url,
                headless=headless,
                debug_url=debug_url,
            ...
            results.append(normalize_crawled_room_price_result(result))
        except (RuntimeError, ValueError) as exc:
            results.append(
                normalize_crawled_room_price_result(
                    {"hotel_name": name, "hotel_url": url, "error": str(exc), "rooms": []}
                )
            )
    return results
...
_ROOM_TREND_CATEGORY_DEFS = (
    ("family", "亲子房", ("亲子", "家庭", "母子", "儿童", "孩童")),
    ("business", "商务房", ("商务", "商旅")),
    ("standard", "标间", ("标间", "标准间", "标准双床", "双床", "双人间", "双床房")),
    ("big_bed", "大床房", ("大床", "单床", "单人间", "queen", "king")),
)
_ROOM_TREND_BUCKET_GAP = timedelta(minutes=90)


def _classify_room_trend_category(room_type: object | None) -> tuple[str, str] | None:
    text_value = str(room_type or "").strip().lower()
    if not text_value:
        return None
    for code, display_name, keywords in _ROOM_TREND_CATEGORY_DEFS:
        if any(keyword in text_value for keyword in keywords):
            return code, display_name
    return None


def _normalize_room_trend_series_type(value: object | None) -> str:
    normalized = str(value or "room_category").strip().lower()
    if normalized in {"hotel", "hotel_min", "hotel_min_price"}:
        return "hotel_min_price"
    if normalized in {"room_category", "category", "room_type"}:
        return "room_category"
    raise ValueError("series_type must be room_category or hotel_min_price")


def _parse_room_trend_collected_at(value: object | None) -> datetime | None:
...
def _format_room_trend_collected_at(value: datetime | object | None) -> str:
    parsed = _parse_room_trend_collected_at(value)
    if parsed is not None:
        return parsed.strftime("%Y-%m-%d %H:%M:%S")
    return str(value or "").strip()


def _build_room_trend_time_buckets(values: list[object]) -> dict[str, dict]:
    parsed_values = sorted(
        {
            _format_room_trend_collected_at(value): _parse_room_trend_collected_at(value)
            for value in values
            if _parse_room_trend_collected_at(value) is not None
        }.items(),
        key=lambda item: item[1],
    )
    buckets: list[dict] = []
    for collected_at, parsed in parsed_values:
        if parsed is None:
            continue
        if not buckets or parsed - buckets[-1]["end_dt"] > _ROOM_TREND_BUCKET_GAP:
            buckets.append(
                {
                    "bucket_at": collected_at,
                    "start_at": collected_at,
                    "end_at": collected_at,
                    "start_dt": parsed,
                    "end_dt": parsed,
                }
            ...
            buckets[-1]["end_at"] = collected_at
            buckets[-1]["end_dt"] = parsed
    ...
    for collected_at, parsed in parsed_values:
        ...
        for bucket in buckets:
            if bucket["start_dt"] <= parsed <= bucket["end_dt"]:
                mapping[collected_at] = {
...
def get_room_price_trend_summary(
    db: Session,
    *,
    shop_id: int,
    days: int = 2,
    hotel_name: str | None = None,
    point_limit: int = 120,
    series_type: str = "room_category",
) -> dict:
    """Build chart-ready min-price trends by room category/hotel or hotel."""
    ensure_room_prices_table(db)

    if days < 1 or days > 30:
        raise ValueError("days must be between 1 and 30")
    if point_limit < 1 or point_limit > 240:
        raise ValueError("point_limit must be between 1 and 240")
    resolved_series_type = _normalize_room_trend_series_type(series_type)
    ...
    if normalized_hotel_name:
        hotel_filter = "AND hotel_name = :hotel_name"
        params["hotel_name"] = normalized_hotel_name
        ...
        rows = db.execute(
            text(
                ...
                WHERE shop_id = :shop_id
                  AND collected_at >= DATE_SUB(NOW(), INTERVAL :days DAY)
                  {hotel_filter}
                ORDER BY collected_at ASC, hotel_name ASC, room_type ASC
                """
            ),
            params,
        ).mappings().all()
    ...
    grouped_points: dict[tuple[str, str], dict] = {}
    latest_collected_at = ""
    point_count = 0
    global_min_price: float | None = None
    global_max_price: float | None = None
    matched_hotels: set[str] = set()
    time_buckets = _build_room_trend_time_buckets([row["collected_at"] for row in rows])

    for row in rows:
        collected_at = _format_room_trend_collected_at(row["collected_at"])
        bucket = time_buckets.get(
            collected_at,
            {
                "bucket_at": collected_at,
                "start_at": collected_at,
        ...
        bucket_at = str(bucket["bucket_at"])
        hotel_name_value = str(row["hotel_name"] or "").strip()
        hotel_url_value = str(row.get("hotel_url") or "").strip()
        price = round(float(row["price"] or 0), 2)
        if price <= 0:
            continue
        if resolved_series_type == "room_category":
            category = _classify_room_trend_category(row["room_type"])
            if category is None:
                continue
            category_code, series_name = category
            hotel_display_name = hotel_name_value or "未知酒店"
            series_code = f"{category_code}|{hotel_display_name}"
            series_meta = {
                "category_code": category_code,
                "category_name": series_name,
                "hotel_name": hotel_display_name,
                "hotel_url": hotel_url_value,
            }
        else:
            if not hotel_name_value:
                continue
            series_code = hotel_name_value
            series_name = hotel_name_value
            series_meta = {
                "hotel_name": hotel_name_value,
                "hotel_url": hotel_url_value,
            }
        matched_hotels.add(hotel_name_value)
        grouped.setdefault(
            series_code,
            series_meta,
        )
        point_key = (series_code, bucket_at)
        point = grouped_points.setdefault(
            point_key,
            {
                "collected_at": bucket_at,
                "collected_at_start": str(bucket["start_at"]),
                "collected_at_end": str(bucket["end_at"]),
                "min_price": price,
                "max_price": price,
                "price_sum": 0.0,
                "sample_count": 0,
        ...
        point["min_price"] = min(float(point["min_price"]), price)
        point["max_price"] = max(float(point["max_price"]), price)
        point["price_sum"] = float(point["price_sum"]) + price
        point["sample_count"] = int(point["sample_count"]) + 1
        if hotel_name_value:
            point["hotel_names"].add(hotel_name_value)

    chart_series: list[dict] = []
    for series_code, series_meta in grouped.items():
        category_points = []
        for (point_series_code, _), raw_point in grouped_points.items():
            if point_series_code != series_code:
                continue
            sample_count = int(raw_point["sample_count"] or 0)
            point = {
                "collected_at": str(raw_point["collected_at"]),
                "collected_at_start": str(raw_point["collected_at_start"]),
                "collected_at_end": str(raw_point["collected_at_end"]),
                "min_price": round(float(raw_point["min_price"] or 0), 2),
                "avg_price": round(float(raw_point["price_sum"] or 0) / max(sample_count, 1), 2),
                "max_price": round(float(raw_point["max_price"] or 0), 2),
                "room_count": sample_count,
                "hotel_count": len(raw_point["hotel_names"]),
            }
            category_points.append(point)

        points = list(sorted(category_points, key=lambda item: str(item["collected_at"])))[-point_limit:]
        if not points:
            continue
        point_count += len(points)
        first_point = points[0]
        last_point = points[-1]
        latest_collected_at = max(latest_collected_at, str(last_point.get("collected_at_end") or last_point["collected_at"]))
        for point in points:
            if global_min_price is None or float(point["min_price"]) < global_min_price:
                global_min_price = float(point["min_price"])
            if global_max_price is None or float(point["min_price"]) > global_max_price:
                global_max_price = float(point["min_price"])
        change_amount = round(float(last_point["min_price"]) - float(first_point["min_price"]), 2)
        change_pct = round(change_amount / float(first_point["min_price"]) * 100, 2) if float(first_point["min_price"]) > 0 else None
        chart_series.append(
            {
                **series_meta,
                "point_count": len(points),
                "latest_collected_at": last_point.get("collected_at_end") or last_point["collected_at"],
                "latest_bucket_at": last_point["collected_at"],
                "latest_min_price": last_point["min_price"],
    ...
    chart_series.sort(
        key=lambda item: (
            ...
            str(item.get("category_code") or item.get("hotel_name") or ""),
        )
    )
    latest_points = [float(item["latest_min_price"]) for item in chart_series if float(item.get("latest_min_price") or 0) > 0]
    ...
    return {
        ...
        "hotel_name": normalized_hotel_name or None,
        "hotel_count": len(matched_hotels),
        "category_count": len(chart_series),
        "point_count": point_count,
        "latest_collected_at": latest_collected_at or None,
        "price_min": round(float(global_min_price or 0), 2) if global_min_price is not None else None,
        "price_max": round(float(global_max_price or 0), 2) if global_max_price is not None else None,
        "latest_price_avg": round(sum(latest_points) / len(latest_points), 2) if latest_points else None,
        "categories": [item.get("category_name") or item.get("hotel_name") for item in chart_series],
        "series": chart_series,
    }


def _build_room_price_trend_advice_prompt(trend_summary: dict) -> str:
    series = list(trend_summary.get("series") or [])
    lines: list[str] = []
    for item in series[:8]:
        points = list(item.get("points") or [])[-8:]
        point_text = ", ".join(
            f"{str(point.get('collected_at') or '')[5:16]} ¥{float(point.get('min_price') or 0):.0f}"
            for point in points
            if float(point.get("min_price") or 0) > 0
        )
        series_name = _format_room_price_trend_series_name(item)
        lines.append(
            f"- {series_name}: 最新最低价 ¥{float(item.get('latest_min_price') or 0):.0f}, "
            f"{_format_trend_window_label(trend_summary.get('days'))}变化 {float(item.get('change_amount') or 0):+.0f} "
            f"({float(item.get('change_pct') or 0):+.1f}%), 轨迹: {point_text}"
        )

    return (
        "你是一名酒店竞对监控分析助手。请基于竞对房型分类最低价趋势，输出简短运营建议。"
        "只返回 JSON，不要 Markdown。格式为："
        '{"summary":"一句总览","items":[{"title":"建议标题","detail":"50字内建议","signal":"up|down|flat"}]}\n'
        f"监控窗口: {_format_trend_window_label(trend_summary.get('days'))}\n"
        f"房型分类数量: {int(trend_summary.get('category_count') or 0)}\n"
        f"最新采集时间: {trend_summary.get('latest_collected_at') or '-'}\n"
        "趋势数据:\n"
        + "\n".join(lines)
    )


def _format_room_price_trend_series_name(item: dict) -> str:
    category_name = str(item.get("category_name") or "").strip()
    hotel_name = str(item.get("hotel_name") or "").strip()
    if category_name and hotel_name:
        return f"{category_name} · {hotel_name}"
    return category_name or hotel_name or "房型"


def _format_trend_window_label(days: object) -> str:
    value = int(days or 2)
    return "48小时" if value == 2 else f"{value}天"


def _normalize_room_price_trend_advice_payload(payload: object) -> dict | None:
    if not isinstance(payload, dict):
        return None
    ...
    for item in raw_items:
        ...
        signal = str(item.get("signal") or "flat").strip().lower()
        if signal not in {"up", "down", "flat"}:
            signal = "flat"
        if not title or not detail:
            continue
        items.append({"title": title[:32], "detail": detail[:120], "signal": signal})
        if len(items) >= 4:
            break
    if not items:
        return None
...
def _call_room_price_trend_advice_llm(prompt: str) -> tuple[str, dict] | None:
    settings = get_settings()
    ...
    payload = {
        ...
        "messages": [
            {"role": "system", "content": "你是酒店收益管理分析助手，只返回 JSON。"},
    ...
    with urlopen(request, timeout=max(5, int(getattr(settings, "llm_timeout_sec", 5) or 5))) as response:
        raw = response.read().decode("utf-8")
    parsed = json.loads(raw)
    content = parsed["choices"][0]["message"]["content"]
    if isinstance(content, list):
        content = "".join(str(part.get("text") or "") for part in content if isinstance(part, dict))
    normalized = _normalize_room_price_trend_advice_payload(json.loads(_extract_json_candidate(str(content or "").strip())))
    if not normalized:
        raise RuntimeError("invalid trend advice payload")
    return provider, normalized


def _build_rule_based_room_price_trend_advice(trend_summary: dict) -> dict:
    series = list(trend_summary.get("series") or [])
    if not series:
        return {
            "source": "rule_based",
            "summary": "暂无竞对历史价格数据，等待下一次自动采集后再生成趋势建议。",
            "items": [],
        }
    ...
    for item in sorted_series[:3]:
        category_name = _format_room_price_trend_series_name(item)
        latest_price = float(item.get("latest_min_price") or 0)
        change_pct = float(item.get("change_pct") or 0)
        change_amount = float(item.get("change_amount") or 0)
        if change_pct <= -5:
            advice_items.append(
                {
                    "title": f"{category_name} 有明显降价",
                    "detail": f"最近窗口最低价下调 {abs(change_pct):.1f}%（约 {abs(change_amount):.0f} 元），建议优先核对该类房型的活动价与库存策略。",
                    "signal": "down",
                }
            )
        elif change_pct >= 5:
            advice_items.append(
                {
                    "title": f"{category_name} 价格持续上探",
                    "detail": f"最近窗口最低价上调 {change_pct:.1f}% 至 ¥{latest_price:.0f}，可评估同类房型是否存在跟涨空间。",
                    "signal": "up",
                }
            )
        else:
            advice_items.append(
                {
                    "title": f"{category_name} 价格相对平稳",
                    "detail": f"最近窗口最低价波动 {change_pct:+.1f}% ，建议继续按 2 小时频率观察异常波动点。",
                    "signal": "flat",
                }
            )

    latest_price_avg = trend_summary.get("latest_price_avg")
    summary = (
        f"最近 {_format_trend_window_label(trend_summary.get('days'))}已累计 {int(trend_summary.get('point_count') or 0)} 个价格点，"
        f"覆盖 {int(trend_summary.get('category_count') or 0)} 类重点房型。"
    )
    if latest_price_avg is not None:
        summary += f" 当前竞对最低价均值约 ¥{float(latest_price_avg):.0f}。"
...
def generate_room_price_trend_advice(trend_summary: dict) -> dict:
    """Generate concise advice for competitor min-price trends."""
    base_result = _build_rule_based_room_price_trend_advice(trend_summary)
    if not list(trend_summary.get("series") or []):
        return base_result

    try:
        llm_result = _call_room_price_trend_advice_llm(_build_room_price_trend_advice_prompt(trend_summary))
    except Exception:
        llm_result = None
    ...
    source, payload = llm_result
    return {
        "source": source,
        "summary": str(payload.get("summary") or base_result.get("summary") or "").strip()[:120],
