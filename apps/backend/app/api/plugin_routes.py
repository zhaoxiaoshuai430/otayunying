...
from app.schemas.pricing import (
    ...
    ManualRoomMappingsReplaceRequest,
...
from app.services.manual_room_mapping_service import list_manual_room_mappings, replace_manual_room_mappings
...
@plugin_bp.route('/manual-room-mappings', methods=['GET', 'POST'])
def plugin_manual_room_mappings() -> dict:
    db = _db()
    context = require_plugin_auth_context()
    if request.method == 'GET':
        shop_id = _q_int('shop_id', default=int(context.current_shop_id))
        try:
            enforce_shop_id_match(int(context.current_shop_id), shop_id)
            items = list_manual_room_mappings(
                db=db,
                tenant_id=int(context.tenant_id),
            ...
            return {'shop_id': int(context.current_shop_id), 'count': len(items), 'items': items}
        except RuntimeError as exc:
            raise ApiError(500, str(exc)) from exc

    try:
        payload = ManualRoomMappingsReplaceRequest.model_validate(_json())
    except ValidationError as exc:
        raise ApiError(400, str(exc)) from exc
    try:
        enforce_shop_id_match(int(context.current_shop_id), payload.shop_id, field_name='payload.shop_id')
        items = replace_manual_room_mappings(
            db=db,
            tenant_id=int(context.tenant_id),
            user_id=int(context.user_id),
            shop_id=int(context.current_shop_id),
        ...
        return {'shop_id': int(context.current_shop_id), 'saved_count': len(items), 'count': len(items), 'items': items}
    except RuntimeError as exc:
        raise ApiError(500, str(exc)) from exc
...
def plugin_merchant_pricing_preview() -> dict:
    ...
    try:
        ...
        return preview_merchant_pricing_recommendations(
            ...
            merchant_items=payload.merchant_items,
            collect_mode=payload.collect_mode,
            debug_url=payload.debug_url,
        )
    except ValueError as exc:
...
@plugin_bp.route('/competitor/room-prices/save', methods=['POST'])
def plugin_competitor_room_prices_save() -> dict:
    db = _db()
    _, tenant_shop_id = require_tenant_shop(db)
    payload = _json()
    payload_shop_id = payload.get('shop_id', tenant_shop_id)
    hotels = payload.get('hotels', payload.get('crawl_results'))
    if not isinstance(hotels, list):
        raise ApiError(400, 'hotels must be a list')

    try:
        enforce_shop_id_match(tenant_shop_id, payload_shop_id, field_name='payload.shop_id')
        saved_count = save_room_prices(db=db, shop_id=tenant_shop_id, crawl_results=hotels)
        total_rooms = sum(
        ...
        return {
            'shop_id': tenant_shop_id,
            'hotel_count': len([item for item in hotels if isinstance(item, dict)]),
            'total_rooms': total_rooms,
...
@plugin_bp.route('/competitor/room-price-trends', methods=['GET'])
def plugin_competitor_room_price_trends() -> dict:
    db = _db()
    _, tenant_shop_id = require_tenant_shop(db)
    shop_id = _q_int('shop_id', default=tenant_shop_id)
    days = _q_bounded_int('days', default=2, minimum=1, maximum=30)
    point_limit = _q_bounded_int('point_limit', default=120, minimum=1, maximum=240)
    hotel_name = _q_str('hotel_name')
    series_type = _q_str('series_type', 'room_category') or 'room_category'
    include_advice = _q_bool('include_advice', default=True)
    ...
    try:
        ...
        if include_advice:
            result['advice'] = generate_room_price_trend_advice(result)
        return result
    except ValueError as exc:
        raise ApiError(400, str(exc)) from exc
    except RuntimeError as exc:
...
def plugin_competitor_workflow_preview() -> dict:
    ...
    try:
        ...
        return preview_competitor_driven_pricing_workflow(
            ...
            headless=payload.headless,
            merchant_items=payload.merchant_items,
            competitor_hotels=payload.competitor_hotels,
            manual_room_mappings=payload.manual_room_mappings,
            inventory_snapshot=payload.inventory_snapshot,
