...
class PluginApiRoutesTestCase(unittest.TestCase):
    ...
    def test_plugin_room_prices_save_uses_service(self) -> None:
        with (
            patch('app.api.plugin_routes.require_tenant_shop', return_value=(1, 1)),
            patch('app.api.plugin_routes.save_room_prices', return_value=6) as mocked_save,
        ):
            response = self.client.post(
                '/plugin/competitor/room-prices/save',
                json={
                    'shop_id': 1,
                    'hotels': [
                        {
                            ...
                            'rooms': [
                                {'room_type': '高级大床房', 'price': 488.0},
                                {'room_type': '豪华双床房', 'price': 520.0},
                            ],
                        },
                        {
                            'hotel_name': '杭州西子湖四季酒店',
                            'hotel_url': 'https://example.com/b',
                            'rooms': [
                                {'room_type': '高级大床房', 'price': 1288.0},
                            ],
                        },
                    ],
                },
    ...
    def test_plugin_room_price_trends_includes_advice(self) -> None:
        trend_payload = {
            'shop_id': 1,
            'days': 7,
            'metric': 'min_price',
            'hotel_count': 1,
            'point_count': 2,
            'latest_collected_at': '2026-04-24 10:00:00',
            'series': [
                {
                    'hotel_name': '杭州君悦酒店',
        ...
        advice_payload = {
            'source': 'rule_based',
            'summary': '最近 7 天价格整体下探。',
            'items': [{'title': '关注降价', 'detail': '建议观察活动波动。', 'signal': 'down'}],
        }
        with (
            patch('app.api.plugin_routes.require_tenant_shop', return_value=(1, 1)),
            patch('app.api.plugin_routes.get_room_price_trend_summary', return_value=dict(trend_payload)) as mocked_summary,
            patch('app.api.plugin_routes.generate_room_price_trend_advice', return_value=advice_payload) as mocked_advice,
        ):
            response = self.client.get('/plugin/competitor/room-price-trends?days=7&include_advice=1', headers=self.headers)

        self.assertEqual(response.status_code, 200)
        payload = response.get_json()
        self.assertEqual(payload['hotel_count'], 1)
        self.assertEqual(payload['advice'], advice_payload)
        mocked_summary.assert_called_once_with(
            ...
            hotel_name=None,
            point_limit=60,
            series_type='room_category',
        )
        mocked_advice.assert_called_once()

    def test_plugin_room_price_trends_rejects_mismatched_shop_id(self) -> None:
        with (
            patch('app.api.plugin_routes.require_tenant_shop', return_value=(1, 1)),
            patch('app.api.plugin_routes.get_room_price_trend_summary') as mocked_summary,
        ):
            response = self.client.get('/plugin/competitor/room-price-trends?shop_id=2', headers=self.headers)

        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.get_json()['detail'], 'shop_id must match X-Shop-Id')
    ...
    def test_plugin_competitor_room_prices_uses_existing_services(self) -> None:
        ...
        self.assertEqual(mocked_crawl.call_args.kwargs['collect_mode'], 'storage_state')
        self.assertEqual(mocked_crawl.call_args.kwargs['storage_state_name'], 'guest-shop-1.json')
        self.assertEqual(mocked_crawl.call_args.kwargs['shop_id'], 1)
    ...
    def test_plugin_competitor_pricing_advice_preview_allows_empty_competitor_hotels(self) -> None:
        expected = {
            'shop_id': 1,
            'recommendation_source': 'fallback',
            'advice_summary': {'suggested_price': 458.0},
        }
        with (
            patch('app.api.plugin_routes.require_tenant_shop', return_value=(1, 1)),
            patch('app.api.plugin_routes.preview_competitor_pricing_advice', return_value=expected) as mocked_preview,
        ...
        self.assertEqual(response.get_json(), expected)
        mocked_preview.assert_called_once()
        self.assertEqual(mocked_preview.call_args.kwargs['competitor_hotels'], [])
