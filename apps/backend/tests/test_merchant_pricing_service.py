...
class MerchantPricingServiceTestCase(unittest.TestCase):
    ...
    @patch('app.services.merchant_pricing_service.create_merchant_pricing_audit')
    @patch('app.services.merchant_pricing_service.preview_competitor_pricing_advice')
    @patch('app.services.merchant_pricing_service.collect_auto_pricing_context')
    @patch('app.services.merchant_pricing_service.preview_merchant_pricing_recommendations')
    def test_preview_competitor_driven_pricing_workflow_uses_manual_room_mapping_price(
        self,
        mocked_preview,
        mocked_context,
        mocked_room_advice,
        ...
        mocked_context.return_value = {
            'rule': {'strategy': 'balanced', 'lookback_days': 7, 'max_change_pct': 8, 'high_risk_change_pct': 10, 'require_manual_approval': False},
            'inventory_snapshot': {'total_rooms': 20, 'available_rooms': 5, 'current_price': 120.0},
        }
        mocked_room_advice.return_value = {
            'recommendation_source': 'manual_room_mappings',
            'advice_summary': {'suggested_price': 188.0},
        ...
        result = preview_competitor_driven_pricing_workflow(
            db=object(),
            shop_id=1,
            manual_room_mappings=[
                {
                    ...
                    'rate_name': '标准价',
                    'current_price': 188.0,
                    'competitor_room_names': ['豪华大床房'],
                }
            ],
            competitor_hotels=[{'hotel_name': '竞对A', 'rooms': [{'room_type': '大床房', 'rate_name': '标准价', 'price': 199.0}]}],
        )

        self.assertEqual(result['items'][0]['current_price'], 188.0)
        self.assertEqual(result['items'][0]['final_price'], 188.0)
        self.assertEqual(mocked_room_advice.call_args.kwargs['manual_room_mappings'][0]['current_price'], 188.0)
        self.assertEqual(mocked_room_advice.call_args.kwargs['manual_room_mappings'][0]['room_type'], '豪华大床房')
        self.assertEqual(result['room_pricing_advice']['room_recommendations'][0]['current_price'], 188.0)
...
class MerchantPricingRouteTestCase(unittest.TestCase):
    ...
    @patch('app.api.plugin_routes.list_manual_room_mappings')
    def test_plugin_manual_room_mappings_list_route(self, mocked_list) -> None:
        mocked_list.return_value = [
            {
                'id': 3,
                'display_name': '高级大床房',
        ...
        app.testing = True
        client = app.test_client()
        context = SimpleNamespace(tenant_id=2, user_id=7, current_shop_id=3)

        with patch('app.api.plugin_routes.require_plugin_auth_context', return_value=context):
            resp = client.get('/plugin/manual-room-mappings?shop_id=3')

        self.assertEqual(resp.status_code, 200)
        data = resp.get_json()
        self.assertEqual(data['shop_id'], 3)
        self.assertEqual(data['count'], 1)
        self.assertEqual(data['items'][0]['display_name'], '高级大床房')
        self.assertEqual(mocked_list.call_args.kwargs['tenant_id'], 2)
        self.assertEqual(mocked_list.call_args.kwargs['user_id'], 7)
        self.assertEqual(mocked_list.call_args.kwargs['shop_id'], 3)

    @patch('app.api.plugin_routes.replace_manual_room_mappings')
    def test_plugin_manual_room_mappings_replace_route(self, mocked_replace) -> None:
        mocked_replace.return_value = [
            {
                'id': 5,
                'display_name': '行政大床房',
                'room_type': '大床房',
        ...
        self.assertEqual(data['items'][0]['current_price'], 529.0)
        self.assertEqual(mocked_replace.call_args.kwargs['tenant_id'], 2)
        self.assertEqual(mocked_replace.call_args.kwargs['user_id'], 7)
        self.assertEqual(mocked_replace.call_args.kwargs['shop_id'], 3)
        self.assertEqual(mocked_replace.call_args.kwargs['items'][0]['display_name'], '行政大床房')
